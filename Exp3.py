...
def decide_std_from_eps(args,Const = 0.1):
    args.std = Const/args.eps
    return args

args = get_GST_params(args)
args = decide_std_from_eps(args)


print(args.dataset)
dataset = get_dataset(args.dataset, sparse=True)
dataset = preprocess_data_from_dataset(dataset)  # pre-process the data
args.num_removes = decide_num_remove(dataset,unlearn_ratio = 0.9)
if args.device>-1:
